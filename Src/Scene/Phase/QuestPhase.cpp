...
void QuestPhase::Draw(void)
{
	...
	{
		DrawFormatString(TURN_MSG_X, TURN_MSG_Y, 0xFFFF00, "現在のターン：%d ", battleTurn_);
		DrawFormatString(TURN_MSG_X, TURN_MSG_Y + 20, 0xFFFF00, "【 BATTLE %d / %d 】", currentWave_, MAX_WAVES);//連戦（Wave）の表示
		DrawFormatString(PLAYER_HP_MSG_X, PLAYER_HP_MSG_Y, 0xFFFFFF, "プレイヤーのHP %d / %d", playerStatus_->hp_, playerStatus_->maxHp_);
