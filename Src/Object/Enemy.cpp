...
void Enemy::ChangeAnim(int command)
{
    switch (command)
    {
    case ACT_1:  currentAnim_ = ENEMY_ANIM::ACT_1; break;
    case ACT_2:  currentAnim_ = ENEMY_ANIM::ACT_2; break;
    case ACT_3:  currentAnim_ = ENEMY_ANIM::ACT_3; break;
    case DAMAGE:  currentAnim_ = ENEMY_ANIM::DAMAGE; break;
    case DEAD:  currentAnim_ = ENEMY_ANIM::DEAD; break;
...
void Enemy::Update()
{
    ...
    if (++currentFrame_ >= totalFrames)
    {
        ...
        //自動で待機(IDLE)に戻す
        else if (currentAnim_ <= ENEMY_ANIM::ACT_3 || currentAnim_ == ENEMY_ANIM::DAMAGE)
        {
            currentAnim_ = ENEMY_ANIM::IDLE;
...
{
    //画面中央付近に表示するための基準座標
    int spawnX = Enemy::DEFAULT_SPAWN_X;
    int spawnY = Enemy::DEFAULT_SPAWN_Y;

    if (turn >= Enemy::TURN_BOSS)
    {
        ...
        //Enemy（名前、体力、攻撃力１、攻撃力２、攻撃力３、素早さ、経験値、攻撃名、攻撃名、攻撃名、BossAnims, spawnX, spawnY）
        return new Enemy("魔王", 999, 43, 37, 30, 50, 0, "こうげき", "魔の威光", "遡行", BossAnims, spawnX, spawnY);
    }
    //17〜20ターン目
    else if (turn >= Enemy::TURN_STAGE_4)
    {
        int enemyType = GetRand(Enemy::RAND_MAX_5_TYPES);
        switch (enemyType)
        {
        case 0: {
    ...
    //13〜16ターン目
    else if (turn >= Enemy::TURN_STAGE_3)
    {
        int enemyType = GetRand(Enemy::RAND_MAX_5_TYPES);
        switch (enemyType)
        {
    ...
    //9〜12ターン目
    else if (turn >= Enemy::TURN_STAGE_2)
    {
        int enemyType = GetRand(Enemy::RAND_MAX_5_TYPES);
        switch (enemyType)
        {
    ...
    //5〜9ターン目
    else if (turn >= Enemy::TURN_STAGE_1)
    {
        int enemyType = GetRand(Enemy::RAND_MAX_5_TYPES);
        switch (enemyType)
        {
    ...
    {
        int enemyType = GetRand(Enemy::RAND_MAX_4_TYPES);
        switch (enemyType)
        {
        case 0:{
